...
from data.datasets.kitti_utils import Calibration, read_label, approx_proj_center, \
    show_heatmap, show_image_with_boxes, show_edge_heatmap
...
class KITTIDataset(Dataset):
    def __init__(self, cfg, transforms=None, is_train=True, split='train', is_visualize=False,
                 augment=True):
        super().__init__()
        ...
        self.split = split
        root = r"E:\fy_works\data\KITTI"
        if self.is_train:
            root_ = root + r"\object\training"
        else:
            if split == "test":
                root_ = root + r"\object\testing"
            else:
                root_ = root + r"\object\training"
        self.image_dir = os.path.join(root_, "image_2")
        self.image_right_dir = os.path.join(root_, "image_3")
        self.label_dir = os.path.join(root_, "label_2")
        self.calib_dir = os.path.join(root_, "calib")

        if split in ["train", "val", "test", "trainval", "train2048", "train2", "val2"]:
            self.imageset_txt = os.path.join(root, "ImageSets", split + ".txt")
        else:
        ...
        # self.flip_prob = 0
        self.flip_prob = 0.5 if is_train else 0
        self.aug_prob = 0.3 if is_train else 0
        self.shift_scale = (0.2, 0.4)

        # whether to use right-view image
        self.use_right_img = cfg.DATASETS.USE_RIGHT_IMAGE & is_train

        self.augmentation = get_composed_augmentations(aug_params=cfg.INPUT.AUG_PARAMS) \
            if (self.is_train and augment) else None
        ...
        # use approximate representations for outside objects
        self.use_approx_center = cfg.INPUT.USE_APPROX_CENTER
        # the type of approximate representations for outside objects
        self.proj_center_mode = cfg.INPUT.APPROX_3D_CENTER
        ...
        print(f"Got {self.__len__()} images to {split}: {self.classes}")
    ...
    def get_edge_utils(self, image_size, pad_size, down_ratio=4):
        ...
        step = 1
        edge_indices = []  # boundary index

        # left
        y = torch.arange(y_min, y_max, step)  # h'
        ...
        # bottom
        x = torch.arange(x_min, x_max, step)  # w'
        ...
        # right
        y = torch.arange(y_max, y_min, -step)  # h'
        ...
        # top
        x = torch.arange(x_max, x_min - 1, -step)  # w'-1
        ...
        # concatenate, (h'+w'+h'+w'-1, 2)
        edge_indices = torch.cat([index.long() for index in edge_indices], dim=0)

        return edge_indices

    def encode_alpha_multibin(self, alpha, num_bin=4, margin=1 / 6):
        # encode alpha (-PI ~ PI) to 2 classes and 1 regression
        encode_alpha = np.zeros(num_bin * 2)  # cls-->0-3, offset-->4-7
        bin_size = 2 * np.pi / num_bin  # pi/2
        margin_size = bin_size * margin  # the overlapped area, pi/12

        bin_centers = self.alpha_centers  # array([0, PI / 2, PI, - PI / 2])
        range_size = bin_size / 2 + margin_size  # pi/4 + pi/12 = pi/3
    ...
    def __getitem__(self, idx):
        if idx >= self.num_samples:
            ...
            # objs = None if not self.is_train else self.get_label_objects(idx)
            objs = self.get_label_objects(idx)

            use_right_img = False

        original_idx = self.image_files[idx][:6]  # '000002.txt'
        ...
        img_w, img_h = img.size
        img, pad_size = self.pad_image(img)  # (pad_x, pad_y)
        ...
        if self.enable_edge_fusion:  # generate edge_indices for the edge fusion module
            input_edge_indices = np.zeros([self.max_edge_length, 2], dtype=np.int64)  # (832, 2)
            edge_indices = self.get_edge_utils((img_w, img_h), pad_size).numpy()  # (h'+w'+h'+w'-1, 2), 顺时针顺序
            ...
            # print(input_edge_count, input_edge_indices.shape, '---------')

        if not self.is_train and not self.is_visualize:
        ...
        offset_3D = np.zeros([self.max_objs, 2], dtype=np.float32)  # offsets from center to expected_center
        # [3] 2d bounding boxes:
        gt_bboxes = np.zeros([self.max_objs, 4], dtype=np.float32)
        bboxes = np.zeros([self.max_objs, 4], dtype=np.float32)
        # [4] key points: 2d coordinates and visible(0/1):
        keypoints = np.zeros([self.max_objs, 10, 3], dtype=np.float32)
        keypoints_depth_mask = np.zeros([self.max_objs, 7], dtype=np.float32)
        ...
        else:  # multi-bin loss: 4-cls + 4-offset
        ...
        for i, obj in enumerate(objs):
            ...
            # bottom center, change the origin to box center
            locs = obj.t.copy()
            ...
            if projected_box2d[0] >= 0 and projected_box2d[1] >= 0 and \
                ...
                box2d = projected_box2d.copy()  # inside
            else:  # min=0, max=h or w
                box2d = obj.box2d.copy()
            '''
            https://github.com/zhangyp15/MonoFlex/issues/2
            The projected 2d boxes are used when all projected corners are inside, 
            otherwise the partial boxes from the original KITTI annotation are used.
            '''

            # filter some unreasonable annotations
            if self.filter_annos:  # [0.9, 20]
                if float_truncation >= self.filter_params[0] and (box2d[2:] - box2d[:2]).min() <= \
                        self.filter_params[1]: continue  # heavily truncated and small box
            ...
            approx_center = False
            if not proj_inside_img:  # outside
            ...
            # 10 keypoints
            bot_top_centers = np.stack((corners_3d[:4].mean(axis=0),
                                        corners_3d[4:].mean(axis=0)), axis=0)  # bottom\top center
            keypoints_3D = np.concatenate((corners_3d, bot_top_centers), axis=0)  # (8+2, 3)
            keypoints_2D, _ = calib.project_rect_to_image(keypoints_3D)  # (8+2, 2)
            ...
            # xyz visible
            keypoints_visible = keypoints_x_visible & keypoints_y_visible & keypoints_z_visible  # 'and'
            # center, diag-02, diag-13
            keypoints_depth_valid = np.stack((keypoints_visible[[8, 9]].all(),  # 'and'
                                              keypoints_visible[[0, 2, 4, 6]].all(),
                                              keypoints_visible[[1, 3, 5, 7]].all(),
                                              keypoints_visible[[0, 1, 4, 5]].all(),
                                              keypoints_visible[[1, 2, 5, 6]].all(),
                                              keypoints_visible[[2, 3, 6, 7]].all(),
                                              keypoints_visible[[3, 0, 7, 4]].all(),)
                                             )  # (3,) or (5, ); [0, 1, 2] or [0, 3, 4, 5, 6]
            if self.use_modify_keypoint_visible:  # True
                keypoints_visible = np.append(
                    np.tile(keypoints_visible[:4] | keypoints_visible[4:8], 2),  # 'or'
            ...
            # clip to the feature map boundary
            target_center[0] = np.clip(target_center[0], x_min, x_max)
            target_center[1] = np.clip(target_center[1], y_min, y_max)

            pred_2D = True
            # In fact, there are some wrong annotations where the projected target center is outside the box2d
            if not (target_center[0] >= box2d[0] and target_center[1] >= box2d[1]
                    and target_center[0] <= box2d[2] and target_center[1] <= box2d[3]):
                pred_2D = False
            ...
            if (bbox_dim > 0).all() and (0 <= target_center[0] <= self.output_width - 1) and (
                ...
                if self.adjust_edge_heatmap and approx_center:
                    # for outside objects, generate 1-D ellipse heatmap
                    bbox_width = min(target_center[0] - box2d[0], box2d[2] - target_center[0])  # =0
                    # bbox_width = bbox_dim[0] / 2  # =0
                    bbox_height = min(target_center[1] - box2d[1], box2d[3] - target_center[1])  # >0
                ...
                else:  # for inside objects, generate circular heatmap
                    radius = gaussian_radius(bbox_dim[1], bbox_dim[0])
                    radius = max(0, int(radius))
                    heat_map[cls_id] = draw_umich_gaussian(heat_map[cls_id], target_center, radius)
                    # heat_map[cls_id] = draw_ellip_gaussian(heat_map[cls_id], target_center, box2d)

                cls_ids[i] = cls_id
                target_centers[i] = target_center  # int format, projected 3d center (in), edge intersection(out)
                # offset due to quantization for inside objects or
                # offset from the intersection to the projected 3D center for outside objects
                offset_3D[i] = proj_center - target_center  # real - int
                ...
                keypoints[i] = np.concatenate((keypoints_2D - target_center.reshape(1, -1),
                                               keypoints_visible[:, np.newaxis]), axis=1)
        ...
        target.add_field("keypoints", keypoints)  # keypoint offset + visible
        target.add_field("keypoints_depth_mask", keypoints_depth_mask)
        target.add_field("dimensions", dimensions)  # (l ,h, w)
        target.add_field("locations", locations)  # (x, y, z)
        target.add_field("calib", calib)
        target.add_field("reg_mask", reg_mask)
        target.add_field("reg_weight", reg_weight)
        target.add_field("offset_3D", offset_3D)  # proj - target_int, (dx, dy)
...
if __name__ == "__main__":
    ...
    tr_dataset = KITTIDataset(cfg, split='train')
    tr_dataset.__getitem__(3713)
