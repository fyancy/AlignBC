...
class Anno_Encoder:
    def __init__(self, cfg, branch: int):
        ...
        self.center_mode = cfg.MODEL.HEAD.CENTER_MODE  # 'max'

        # depth related
        # self.depth_mode = cfg.MODEL.HEAD.DEPTH_MODE  # 'inv_sigmoid'
        self.depth_mode = 'inv_sigmoid' if branch == 1 else 'linear'
        self.depth_range = cfg.MODEL.HEAD.DEPTH_RANGE  # [0.1, 100]
    ...
    def decode_depth(self, depths_offset):
        ...
        elif self.depth_mode == 'inv_sigmoid':  # used
    ...
    def decode_location_flatten(self, points, offsets, depths, calibs, pad_size, batch_idxs):
        ...
        gts = torch.unique(batch_idxs, sorted=True).tolist()  # B images, batch size
        locations = points.new_zeros(points.shape[0], 3).float()  # (N, 3)
        points = (points + offsets) * self.down_ratio - pad_size[batch_idxs]

        for idx, gt in enumerate(gts):
            # corr_pts_idx = torch.nonzero(batch_idxs==gt).squeeze(-1)
            corr_pts_idx = (batch_idxs == gt).nonzero().squeeze(-1)
            calib = calibs[gt]
    ...
    def decode_depth_from_keypoints_batch(self, pred_keypoints, pred_dimensions, calibs, batch_idxs=None):
        # pred_keypoints: K x 10 x 2, 8 vertices, bottom center and top center
        pred_height_3D = pred_dimensions[:, 1].clone()  # l, h, w, (N, 3)->(N,), real physical height
        batch_size = len(calibs)
        if batch_size == 1:
        ...
        always_diag = False
        always_adj = False
        assert not (always_adj and always_diag)

        if self.branch == 1 or always_diag:  # diagonal
            center_height = pred_keypoints[:, -2, 1] - pred_keypoints[:, -1, 1]  # bottom_y - top_y, (N,)
            corner_02_height = pred_keypoints[:, [0, 2], 1] - pred_keypoints[:, [4, 6], 1]  # (N, 2)
            corner_13_height = pred_keypoints[:, [1, 3], 1] - pred_keypoints[:, [5, 7], 1]  # (N, 2)
        ...
        elif self.branch == 2 or always_adj:  # adjacent
            center_height = pred_keypoints[:, -2, 1] - pred_keypoints[:, -1, 1]  # bottom_y - top_y, (N,)
            corner_01_height = pred_keypoints[:, [0, 1], 1] - pred_keypoints[:, [4, 5], 1]  # (N, 2)
            corner_12_height = pred_keypoints[:, [1, 2], 1] - pred_keypoints[:, [5, 6], 1]  # (N, 2)
            corner_23_height = pred_keypoints[:, [2, 3], 1] - pred_keypoints[:, [6, 7], 1]  # (N, 2)
            corner_30_height = pred_keypoints[:, [3, 0], 1] - pred_keypoints[:, [7, 4], 1]  # (N, 2)
            pred_keypoint_depths = {'center': [], 'corner_01': [], 'corner_12': [],
                                    'corner_23': [], 'corner_30': []}
            ...
            for idx, gt_idx in enumerate(torch.unique(batch_idxs, sorted=True).tolist()):
                ...
                center_depth = calib.f_u * pred_height_3D[corr_pts_idx] / (
                        F.relu(center_height[corr_pts_idx]) * self.down_ratio + self.EPS)
                corner_01_depth = calib.f_u * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (
                        F.relu(corner_01_height[corr_pts_idx]) * self.down_ratio + self.EPS)
                corner_12_depth = calib.f_u * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (
                        F.relu(corner_12_height[corr_pts_idx]) * self.down_ratio + self.EPS)
                corner_23_depth = calib.f_u * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (
                        F.relu(corner_23_height[corr_pts_idx]) * self.down_ratio + self.EPS)
                corner_30_depth = calib.f_u * pred_height_3D[corr_pts_idx].unsqueeze(-1) / (
                        F.relu(corner_30_height[corr_pts_idx]) * self.down_ratio + self.EPS)

                corner_01_depth = corner_01_depth.mean(dim=1)
                corner_12_depth = corner_12_depth.mean(dim=1)
                corner_23_depth = corner_23_depth.mean(dim=1)
                corner_30_depth = corner_30_depth.mean(dim=1)

                pred_keypoint_depths['center'].append(center_depth)
                pred_keypoint_depths['corner_01'].append(corner_01_depth)
                pred_keypoint_depths['corner_12'].append(corner_12_depth)
                pred_keypoint_depths['corner_23'].append(corner_23_depth)
                pred_keypoint_depths['corner_30'].append(corner_30_depth)

        for key, depths in pred_keypoint_depths.items():
            pred_keypoint_depths[key] = torch.clamp(torch.cat(depths),  # Tensor (N, )
                                                    min=self.depth_range[0], max=self.depth_range[1])

        pred_depths = torch.stack([depth for depth in pred_keypoint_depths.values()], dim=1)  # (N, 3 or 5)
    ...
    def decode_axes_orientation(self, vector_ori, locations):
        ...
        """
        if self.multibin:
            pred_bin_cls = vector_ori[:, : self.orien_bin_size * 2].view(-1, self.orien_bin_size, 2)  # (N, 4, 2)
            pred_bin_cls = torch.softmax(pred_bin_cls, dim=2)[..., 1]  # (N, 4)
            orientations = vector_ori.new_zeros(vector_ori.shape[0])  # (N,)
            for i in range(self.orien_bin_size):
                mask_i = (pred_bin_cls.argmax(dim=1) == i)  # (N, ), bool
                s = self.orien_bin_size * 2 + i * 2  # [:8]->cls, [8:]->offset, 8+[0, 2, 4, 6]
                e = s + 2  # 8 + [2, 4, 6, 8]
                pred_bin_offset = vector_ori[mask_i, s: e]
                orientations[mask_i] = torch.atan2(pred_bin_offset[:, 0],
